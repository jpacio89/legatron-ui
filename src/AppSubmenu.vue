<template>
	<ul v-if="items" role="menu">
		<template v-for="(item,i) of items">
			<li :key="item.label || i" role="none" v-if="visible(item) && !item.separator"
				:class="[{'active-rootmenuitem':activeIndex === i && root && layoutMode === 'horizontal' && !item.disabled,
			...
			(activeIndex === i && layoutMode !== 'horizontal') || (activeIndex === i && !root && layoutMode === 'horizontal')) && !item.disabled}]">
				...
				<transition name="layout-submenu-container">
					<appsubmenu v-show="activeIndex === i" :items="visible(item) && item.items" :menuActive="menuActive" :layoutMode="layoutMode" :parentMenuItemActive="activeIndex === i"
